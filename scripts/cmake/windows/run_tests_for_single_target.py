...
def runTests(cfg_details: dict[str, str], build_folder_path: Path, executable_name: str) -> None:
    ...
    if cfg_details["Toolchain"] == CfgFileValuesMapOfMap["Toolchain"]["Ninja (clang)"]:
        executable_path = build_folder_path / f"{executable_name}.exe"
        ...
        if cfg_details["Build mode"] == CfgFileValuesMapOfMap["Build mode"]["Release"]:
            executable_path = build_folder_path / "Release" / f"{executable_name}.exe"
        elif cfg_details["Build mode"] == CfgFileValuesMapOfMap["Build mode"]["Debug"]:
            executable_path = build_folder_path / "Debug" / f"{executable_name}.exe"
