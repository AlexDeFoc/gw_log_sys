...
def requestPlatformFromUser() -> None:
    ...
    if platform_chosen == "Windows":
        if build_system_chosen == "CMake":
            run([executable, str(current_script_folder_path / "cmake" / "windows" / "run_all_scripts_for_all_targets_without_tests.py")])
    elif platform_chosen == "Linux":
        if build_system_chosen == "CMake":
            run([executable, str(current_script_folder_path / "cmake" / "linux" / "run_all_scripts_for_all_targets_without_tests.py")])
