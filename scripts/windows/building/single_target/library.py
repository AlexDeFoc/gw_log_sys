...
def main() -> None:
    ...
    commands_list: Final[dict[str, str]] = constructCommands(build_system_folder_path,
                                                             build_folder_path,
    ...
    important_item_paths: list[Path] = getImportantItemPathsList(build_folder_path,
                                                                 chosen_generator, chosen_compiler,
                                                                 chosen_library_type, chosen_build_mode,
    ...
    if chosen_generator == "Visual Studio 2022":
        if chosen_compiler == "MSVC":
            ...
            if chosen_build_mode == "Release":
                ...
                important_item_paths.append(build_folder_path / f"{build_mode_path_segment}" / f"{project_name}.pdb")
        ...
        if chosen_compiler == "Clang (msvc)":
            ...
            if chosen_build_mode == "Debug":
                important_item_paths.append(build_folder_path / f"{project_name}.pdb")
...
def runCommands(chosen_build_system: str, commands_list: dict[str, str]) -> None:
    if chosen_build_system == "CMake":
        run(commands_list["Configuration"], check=True, shell=True)
        run(commands_list["Building"], check=True, shell=True)
...
def constructCommands(build_system_folder_path: Path,
                      ...
                      chosen_build_mode: str, chosen_library_type: str,
                      chosen_build_tests_status: str) -> dict[str, str]:
    commands_list: dict[str, str] = {}
    current_command: str = ""

    if chosen_build_system == "CMake":
        current_command += f"cmake -S \"{str(build_system_folder_path)}\" -B \"{str(build_folder_path)}\""

        if chosen_library_type == "Static":
            current_command += " -DBUILD_STATIC_LIB__GW_LOG_SYS=TRUE"
        else:
            current_command += " -DBUILD_STATIC_LIB__GW_LOG_SYS=FALSE"

        if chosen_build_tests_status == "Yes":
            current_command += " -DBUILD_TESTS__GW_LOG_SYS=TRUE"
        else:
